...
namespace ChiliGames.VRClassroom {
    ...
    public class PlatformManager : MonoBehaviourPunCallbacks {
        ...
        void Awake() {
            ...
            if(PlayFabLogin.UserData.Role == "Teacher")
            {
                mode = Mode.Teacher;
            }
            else if(PlayFabLogin.UserData.Role == "Student")
            {
                mode = Mode.StudentVR;
            }
        ...
        private void Start() {
            ...
            if (mode == Mode.Teacher) {
                ...
                teacherRig.GetComponent<PhotonView>().RequestOwnership();
        ...
        void CreateTeacherBody() {
            PhotonNetwork.Instantiate(teacherBody.name, transform.position, transform.rotation);
